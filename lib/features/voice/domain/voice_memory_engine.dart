...
/// Smart Business Memory — enriches voice drafts from on-device patterns.
final class VoiceMemoryEngine {
  VoiceMemoryEngine({
    required VoiceBusinessMemoryStore store,
    RuleVoiceParser? parser,
  })  : _store = store,
        _parser = parser ?? RuleVoiceParser();

  final VoiceBusinessMemoryStore _store;
  final RuleVoiceParser _parser;
  ...
  Future<VoiceEnrichedResult> enrich({
    ...
    if (draft.paymentBreakdown.paidTotal <= 0 && draft.partyName != null) {
      final ctx = _store.findLastForParty(
        draft.partyName!,
        intent: draft.intent,
      );
      final channel = ctx?.primaryPaymentChannel;
      if (channel != null && ctx != null) {
        final breakdown = switch (channel) {
          'upi' => const PaymentBreakdown(upi: 0),
          'bank' => const PaymentBreakdown(bank: 0),
          _ => const PaymentBreakdown(cash: 0),
        };
        draft = draft.copyWith(paymentBreakdown: breakdown);
        memoryUsed = true;
      }
    }
    ...
    if (_hasReminderKeyword(normalized) && draft.reminderDate == null) {
      ...
      if (ctx?.reminderDay != null && draft.partyName != null) {
        final now = DateTime.now();
        var month = now.month;
        var year = now.year;
        var day = ctx!.reminderDay!;
        if (day < now.day) {
          month += 1;
          if (month > 12) {
            month = 1;
        ...
        draft = draft.copyWith(reminderDate: DateTime(year, month, day));
        confidence[VoiceConfidenceField.reminder] = VoiceConfidenceLevel.medium;
        memoryUsed = true;
      }
    }
